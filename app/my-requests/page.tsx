// approval_flow_frontend/app/my-requests/page.tsx
"use client"
...
import { Clock, CheckCircle, XCircle, FileText, Calendar, User, Eye, Search } from "lucide-react"
...
export default function MyRequestsPage() {
  ...
  return (
    <SidebarProvider>
      <AppSidebar />
      <SidebarInset>
        <AppHeader title="Mis Solicitudes" breadcrumbs={[{ label: "Mis Solicitudes" }]} />
        <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-foreground">Mis Solicitudes</h1>
              <p className="text-muted-foreground">Todas las solicitudes que has creado</p>
            </div>
          </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Historial completo de tus solicitudes</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {isLoading ? (
                <p>Cargando solicitudes...</p>
              ) : requests.length === 0 ? (
                 <p>No has creado ninguna solicitud todavía.</p>
              ) : (
                  ...
                  <div key={request.id_solicitud} className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/50">
                    <div className="space-y-2 flex-1">
                      ...
                      <div className="flex items-center gap-6 text-xs text-muted-foreground">
                        ...
                        <span className="flex items-center gap-1"><User className="w-3 h-3" />Aprobador: {request.aprobadores.map((a:any) => a.nombre).join(', ')}</span>
                      </div>
                    </div>
